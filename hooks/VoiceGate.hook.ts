...
function isMainSession(sessionId: string): boolean {
  ...
  const sessionsDir = join(paiDir, 'MEMORY', 'STATE', 'kitty-sessions');
  // If no sessions directory or it's empty, assume main session (no subagent tracking active)
  if (!existsSync(sessionsDir)) return true;
  // If session file exists, it's main. If no files exist at all, default to main.
  const hasAnyFiles = readdirSync(sessionsDir).length > 0;
  if (!hasAnyFiles) return true;
  return existsSync(join(sessionsDir, `${sessionId}.json`));
}
